...
### K-means
```{python}
X = df_cleaned[['ra', 'dec', 'parallax', 'pmra', 'pmdec']]
...
best_cluster = avg_silhouettes.index(max(avg_silhouettes))
print(f"highest value = {max(avg_silhouettes)}, \
    n_clusters = {range_n_clusters[best_cluster]}")
...
ax2.scatter(df_cleaned['l'], df_cleaned['b'], c=cluster_labels)
plt.gca().invert_xaxis()
...
ax1.set_xlabel('k')
ax1.set_ylabel('Average Silhouette Score', fontsize=12)
ax1.set_title('Método da Silhueta (com dados Normalizados)')

# Gráfico de Clusters
# Plota as posições (l, b) coloridas pelos novos labels
ax2.scatter(df_cleaned['l'], df_cleaned['b'], c=cluster_labels) 
plt.gca().invert_xaxis()
ax2.set_xlabel('l (°)', fontsize=12)
ax2.set_ylabel('b (°)', fontsize=12)
ax2.set_title(f'Clusters K-Means (k={best_n_clusters})')
...
# Separa os dados de ruído (fundo) e os dados dos clusters
labels = df_cleaned['cluster_hdbscan']
noise_data = df_cleaned[labels == -1]

# Pega os IDs dos clusters reais (ignora o -1)
cluster_ids = sorted([c for c in labels.unique() if c >= 0])

print(f"Gerando CMDs individuais para {len(cluster_ids)} clusters...")

# Itera sobre cada cluster e cria um gráfico para ele
for cluster_id in cluster_ids:
    
    # Seleciona os dados apenas para este cluster
    cluster_data = df_cleaned[labels == cluster_id]
    
    plt.figure(figsize=(8, 10))
    
    # 1. Plota o "campo" de ruído em cinza transparente
    plt.scatter(
        noise_data['bp_rp'], 
        noise_data['phot_g_mean_mag'], 
        c='gray', 
        s=1, 
        alpha=0.1,  # Transparente
        label='Ruído (Campo)'
    )
    
    # 2. Plota os membros do cluster por cima, em laranja
    plt.scatter(
        cluster_data['bp_rp'], 
        cluster_data['phot_g_mean_mag'], 
        c='orange', 
        s=15, 
        label=f'Cluster {cluster_id} ({len(cluster_data)} estrelas)',
        edgecolor='black',
        linewidth=0.5
    )
    
    # --- Customização ---
    plt.title(f'Diagrama Cor-Magnitude (CMD) - Cluster {cluster_id}', fontsize=16)
    plt.xlabel('Cor ($G_{BP} - G_{RP}$)', fontsize=12)
    plt.ylabel('Magnitude G', fontsize=12)
    
    # Inverte o eixo Y (fundamental para CMDs)
    plt.gca().invert_yaxis()
    
    # Define limites (opcional, mas ajuda a comparar)
    # plt.xlim(-1, 5)
    # plt.ylim(20, 5)
...
fig.suptitle(f'Análise de Clusters HDBSCAN - Total: {n_clusters}', fontsize=20, y=1.02)
...
fig.suptitle(f'Análise de CMDs - DBSCAN (Total: {n_clusters} clusters)', fontsize=20, y=1.02)
plt.show()
```
...
fig.suptitle(f'Análise de CMDs - K-Means (Total: {n_clusters} clusters)', fontsize=20, y=1.02)
plt.show()
```
...
fig.suptitle(f'Análise de Movimento Próprio - K-Means (Total: {n_clusters} clusters)', fontsize=20, y=1.02)
plt.show()
```
...
fig.suptitle(f'Análise de Movimento Próprio - DBSCAN (Total: {n_clusters} clusters)', fontsize=20, y=1.02)
plt.show()
```
...
fig.suptitle(f'Análise de Movimento Próprio - HDBSCAN (Total: {n_clusters} clusters)', fontsize=20, y=1.02)
plt.show()
```
...
fig.suptitle(f'Histogramas de Paralaxe - K-Means (Total: {n_clusters} clusters)', fontsize=20, y=1.03)
plt.show()
```
...
fig.suptitle(f'Histogramas de Paralaxe - DBSCAN', fontsize=20, y=1.03)
plt.show()
```
...
fig.suptitle(f'Histogramas de Paralaxe - HDBSCAN', fontsize=20, y=1.03)
plt.show()
```